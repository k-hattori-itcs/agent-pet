...
public class PetManager
{
    ...
    public void Setup()
    {
        ...
        if (!_pets.Any(pet => pet.Id.Equals(_activePetId, StringComparison.OrdinalIgnoreCase)))
        {
            _activePetId = _pets.FirstOrDefault(pet => pet.Id.Equals("koharu", StringComparison.OrdinalIgnoreCase))?.Id
                ?? _pets.OrderBy(pet => pet.Id, StringComparer.OrdinalIgnoreCase).FirstOrDefault()?.Id
                ?? "";
        }
    }
    ...
    public string? ImportPet(string zipPath)
    {
        ...
        {
            return "キャラクターパッケージが破損しているか、対応していない形式です。";
        }
        catch (Exception ex)
        {
        ...
        out ZipArchiveEntry? spriteEntry)
    {
        ...
        if (archive.Entries.Count is 0 or > MaxEntries)
            return "キャラクターパッケージ内のファイル数が上限を超えています。";
        ...
        ref long extractedBytes)
    {
        ...
        while (true)
        {
            ...
            if (entryBytes > maxEntryBytes || extractedBytes > MaxExpandedBytes)
                throw new InvalidDataException("Extracted character data exceeded the configured limit.");
    ...
    private static PetInfo? LoadPetInfo(string dir)
    {
        ...
        {
            AppLogger.Error("キャラクター定義 pet.json の読み込みに失敗しました。", ex);
            return null;
        }
    }
}
