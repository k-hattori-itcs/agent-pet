...
AgentCompanion reads local Codex and Claude Code history and can optionally run a loopback-only API proxy. Imported character packages and proxy target settings are treated as untrusted input. The proxy is not intended to be exposed outside the local machine.
